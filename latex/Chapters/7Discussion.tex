...
%*****************************************
\todo{kann man auch aufsplitten in discussion und future work}
\todo{mir fehlt in der diskussion noch die problematik, (siehe mail letztens), dass im kontext des buches bestimmte begriffe anders definiert werden als im allgemeinen sprachgebrauch.
bzw. dass das modell fragen aus seinem vorwissen beantwortet und nicht aus dem buch.
für AW ist das ein wichtiges kriterium.
typische beispiele: daten, information, wissen, informationssystem (schließt menschen mit ein), softwareprodukt vs anwendungssystem.
typische fangfrage: Ist i.s.h.med ein Anwendungssystem? Antwort: Nein, ein Softwareprodukt.
kam das problem bei deiner evaluation nicht vor?
vielleicht bräuchte man da eine spezielle fragenkategorie um genau das zu prüfen. das sprengt dann wohl den rahmen deiner arbeit aber du könntest es bei future work erwähnen.
}
...
\todo{finde ich sehr gut, dass du die probleme nicht wie manch andere unter den teppich kehrst sondern hervorhebst. 0,3 ist ja auch schon ein beachtliches ergebnis und eine gute ansatzstelle für zukünftige forschung.}
...
Das Llama 2 Modell wurde ohne speziellen Fokus auf eine Domäne, insbesondere die Domäne Medizin, trainiert.\todo{medizininformatik. ein medizin modell würde ja noch mal ganz andere sachen lernen.}
...
\todo{würde hierfür einen separaten abschnitt einführen, weil es ja ab hier nicht um pure extraktion geht}
...
\todo{wenn dieer ansatz die besten ergebnisse verspricht, sollte das meiner meinung nach auch an anderer stelle nochmal erwähnt werden zb bei der zusammenfassung.
gut wäre auch eine kleine future work tabelle mit 3 spalten: ansatz (die section titel hier), aufwand und erfolgseinschätzung}
...
\todo{durchaus denkbar finde ich untertrieben da das ja ein weit verbreiteter ansatz ist, da können ruhig noch ein paar mehr quellen dazu rein}
\todo{langchain erwähnen? oder ist das zu speziell?}
\todo{du kannst gerne mal den prototyp dieses ansatzes unter https://github.com/konradHoeffner/localgpt ausprobieren}
\todo{ist vielleicht erwähnenswert dass man mit diesem ansatz auch ressourcen sparen kann, da man für training mehr RAM braucht, das heißt falls man keine separate hardware für training und inferenz zur verfügung hat, dann kann man eventuell ein leistungsfähigeres modell einsetzen.}
\todo{als weiteren ansatz könnte ich mir vorstellen, das komplette buch als kontext für die frage vorzugeben. dafür müsste man eventuell das buch kürzen und ein modell mit sehr großer kontext size nehmen. allerdings erwähnst du ja bereits dass dann eher anfang und ende erinnert werden aber die mitte oft vergessen wird.}
\todo{https://huggingface.co/papers/2306.15595 zitieren?}
\todo{praktisch bei diesem ansatz ist, dass das instruct modell wesentlich kleiner sein kann als das hauptmodell, und das man ein multilinguales instruct modell benutzen kann, was für uns sehr relevant ist, da viele fragen auch in deutsch formuliert sind}
