...
Anschließend kann ein von den Autoren vorbereitetes Skript verwendet werden, um die trainierten Gewichte des Modells herunterzuladen.
...
\todo{bist du sicher, dass llama 2 nicht bereits in einem verwendbaren Format von huggingface herunterladbar ist?}
...
\todo{main branch link + abrufdatum macht für git Dateien nicht so viel Sinn, du kannst ja auch direkt einen festen commit verlinken}
...
\todo{an der stelle würde ich das zugehörige paper zitieren}
...
\todo{vielleicht noch dazuschreiben wieviele tokens es vorher waren und was das für deinen Ansatz bedeutet (eventuell auch auf diskussion verweisen :-).
Meine naive Annahme ist, dass für den verbreiteten Ansatz, separate Embeddings zu verwenden (wie bei localGPT und so) ein größerer Kontext sehr wichtig für die Beantwortung von komplexen Fragen ist.
Aber bei deinem Ansatz mit dem erweiterten Finetuning bin ich nicht sicher, ob ein größerer Kontext uns überhaupt etwas bringt}
...
\todo{das finde ich etwas ungünstig zu lesen, dass erst schritte eingeführt und dann wieder verworfen werden, nimm mindestens schritt 2 doch ganz raus}
Außerdem ermöglicht Huggingface nun die Nutzung der Llama\todo{consistent capitalization} 2 Modelle ohne Download und Konvertierung, so dass die ersten beiden Schritte entfallen.
\todo{ich würde an deiner stelle mal (kann auch am Ende sein) mit einem Spellchecker wie aspell durchgehen, obwohl der Llama vielleicht gar nicht anmerkt da das Tier so geschrieben wird, obwohl im Deutschen darf man es glaube ich nicht mit 2 L schreiben}
...
\todo{ich hatte die von thebloke benutzt und glaube ich brauchte da keinen auth token}
...
\todo{ist ne stilfrage aber ich finde texttt für solche Paramameternamen und ähnliches schöner als Anführungszeichen}
...
\todo{eine Webseite zu zitieren ist hier nicht optimal. unten auf https://pypi.org/project/datasets/ steht doch wie man das in bibtex am besten zitiert}
...
\todo{hierzu war doch noch die frage, ob das bei einem Buch überhaupt so aussagekräftig ist, wenn keine Wiederholungen drin sind}
...
\todo{Die Zehnerpotenzen in der Tabelle training-config würde ich so \num{1e-10} setzen. und dann die Zeile wo output-exponent-marker als e gesetzt wird löschen in der Konfiguration wenn du lieber die ausführliche Schreibweise willst. Aber kannst es auch so lassen.
Und eventuell mehr beschreibenden Text zu den Parametern aus dem Fließtext raus und in die Tabellencaption rein (shortcaption aber definieren für die Verzeichnisse). Die Tabelle nimmt sowieso ne ganze Seite in Anspruch. Das fände ich etwas aufgeräumter.
Das mit dem "true | false" und "false | true" zu erklären würde zb gut in die caption passen.}
\begin{table}
    \centering
    \begin{tabular}{ll}
        \toprule
...
\end{table}
\todo{ich würde dir echt raten, jede Dezimalzahl in deiner Arbeit mit dem num Macro von siunitx zu setzen. Jedesmal wenn ich in die Arbeit oder einen Vortrag sehe, sehe ich sowohl Zahlen mit Komma als auch mit Punkt als Trenner. Auch die Abstände und Striche dürften dann konsistent sein.}
...
\todo{anhang wird bei mir als .1 angezeigt}
Eine vollständige Liste der Parameter ist im \cref{app:sec:trainingparameter} zu finden. Die beschriebenen Parameter sind in \cref{tab:training-config} aufgeführt.\\
...
\todo{Da wir keine Mathematiker sind, würde ich in einer Klammer oder Fußnote kurz klarstellen, was eine L2-Norm ist. Heißt die überhaupt auf Deutsch so? Ist der Gradient der Vektor und das ist dann die euklidische Norm, also der Betrag?}
\todo{Das liest sich alles ziemlich trocken, ab und zu mal ein Bild würde es um einiges auflockern. Zum Beispiel ein Gradient lässt sich doch sicher gut als Bild darstellen, kann ja auch ein bereits existierendes Bild mit Quelle sein.
Bei Gradient und generell in diesem Kapitel könnte man auch gut auf das Grundlagenkapitel verweisen.}
...
Diese Bibliothek wurde auch im Artikel von \citet{llama} verwendet und wurde von Google veröffentlicht\footnote{\url{https://github.com/google/sentencepiece} abgerufen am 19.8.2023}.\\
...
\todo{als textblock unübersichtlich. eine tabelle wäre besser}
...
\todo{spellchecker wie aspell mal durchlaufen lassen}
Insgesammt wurden 6 Modelle trainiert und 8 Modelle evaluiert.
\todo{eventuell tabelle besser}
Folgende Modelle wurden trainiert:
\begin{itemize}
    \item Llama 2 7B mit 1 Epoche auf Nvidia V100 Grafikkarten: \enquote{Llama\_$1$e\_v$100$}
    \item Llama 2 7B mit 1 Epoche auf Nvidia A30 Grafikkarten: \enquote{Llama\_$1$e\_a$30$}
    \item Llama 2 7B mit 3 Epochen auf Nvidia V100 Grafikkarten: \enquote{Llama\_$3$e\_v$100$}
    \item Llama 2 7B mit 3 Epochen auf Nvidia A30 Grafikkarten: \enquote{Llama\_$3$e\_a$30$}
    \item Llama 2 7B mit 5 Epochen auf Nvidia A30 Grafikkarten: \enquote{Llama\_$5$e\_a$30$}
    \item Llama 2 7B mit 10 Epochen auf Nvidia A30 Grafikkarten: \enquote{Llama\_$10$e\_a$30$}
\end{itemize}
...
\todo{ich dachte llama 2 33b gibt es nicht, oder kam das nun raus}
...
\todo{ist das dann 8bit float oder integer oder was für ein datentyp?}
...
Alle Kriterien wurden zusätzlich für jeden Fragentyp und jede Fragequelle ausgewertet.
