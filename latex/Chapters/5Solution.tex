...
Die Generierung von Antworten zum erstellen Evaluierungsdatensatz erfolgte in drei verschiedenen Formen.
Zur Generierung von Text durch das GPT4 Modell wurde die offizielle Webseite der OpenAI Foundation\footnote{\url{https://chat.openai.com/} abgerufen am 19.8.2023} verwendet.
Die Nutzung des untrainierten Llama Modells als auch den trainierten Modellen mit V100 Grafikkarten erfolgte mit Hilfe eines eigens erstellen Skriptes.
Diser Skript wurde größtenteils für die Nutzung der Modelle trainiert mit A30 Grafikkarten genutzt, musste jedoch abgewandelt werden, da die Generierung ausschließlich auf einer \ac{gpu} erfolgt, die A30 Grafikkarten mit \SI{24}{\giga\byte} \ac{ram} jedoch das Modell nicht vollständig halten konnten.

\subsection{Erstellung des Evaluierungsdatensatz}\label{subsec:eval-dataset}
Vor der Generierung der Antworten mussten die gestellten Fragen der gegebenen Klausuren aus \cref{sec:approach:questions} umgewandelt werden.
Hierzu wurden die genannten Fragen mit erforderlichem Kontext ergänzt, wenn dieser in der Frage referenziert wurde oder auf Ergebnissen der vorherigen Fragen aufbaut.
Zusätzlich wurden alle Fragen in die englische Sprache übersetzt, da das Llama Modell nahezu ausschließlich auf englischem Text trainiert wurde und das verwendeted Buch \citet{bb} ebenfalls in englischer Sprache verfasst ist.
Jede Frage erhielt 6 zugewiesene Eigenschaften.
\begin{enumerate}
    \item \textbf{question}: Die Originalfrage aus der Quelle
    \item \textbf{transformed}: Die umformulierte und potentiell übersetzte Frage
    \item \textbf{true\_answer}: Die zu erwartende korrekte Antwort
    \item \textbf{num\_answers}: Die enthaltenen Antworten der zu erwartenenden korrekten Antwort
    \item \textbf{source}: Eine Abkürzung für die Quelle der Frage
    \item \textbf{context}: Ein optionaler Kontext, welcher zur Beantwortung der Frage benötigt wird
\end{enumerate}

Mit Hilfe dieser Struktur wurden die Fragen in eine weitere explizite Frageform umgewandelt.
Wie bereits in \cref{sec:approach:questions} beschrieben, ist das Llama Modell nicht mit Hilfe von Reinforcement Training auf die Beantwortung von Fragen trainiert.
Aus diesem Grund wurde Anweisung, Frage und Kontext in einen Textblock zusammengefasst.
Die Struktur ist in \cref{fig:llama-input} dargestellt.
Der verwendete Evaluierungsdatensatz ist im Anhang enthalten.\\
...
Die generierten Fragen wurden vor der Benutzung in die drei \enquote{Single}, \enquote{multi} und \enquote{transfer} Fragearten in Dateien aufgeteilt.
Zusätzlich wurden alle Fragen in einen zweiten Datensatz kopiert, wobei die \enquote{transformed} Fragen mit kleineren gramatikalischen und Rechtschreibfehlern versehen wurden.
Dieser zweite Datensatz wird im gleichen Sinne im folgenden Schritt unter \cref{sec:answer-rating} verwendet, um in der Evaluation das Kriterium \enquote{Robustheit} zu bewerten.

\section{Bewertung generierter Antworten}\label{sec:answer-rating}
Die Bewertung der generierten Antworten erfolgte per Hand.
Die von den Modellen generierten Ausgaben wurden im gleichen Sinne wie in \cref{subsec:eval-dataset} strukturiert, erhielten jedoch drei zusätzliche Eigenschaften.
\begin{enumerate}
    \item \textbf{generated}: Repräsentiert die generierte Antwort des Modells
    \item \textbf{type}: Zeigt den Typ der Frage an
    \item \textbf{true\_input}: Enthält die tatsächliche genutzte Eingabe
\end{enumerate}

Bei der Bewertung wurde zu erst eingeschätzt, ob die generierte Antwort inhaltlichen Bezug zur Frage hat.
Sollte dies nicht der Fall sein, wird diese Frage als \enquote{unbeantwortet} gewertet und enthält ebenso keine Antworten.
Sollte die Antwort inhaltlichen Bezug zur Frage haben, wird die Antwort auf ihre Korrektheit bewertet.
Hierzu gibt die Bewertung eine Anzahl an richtigen Antworten in der generierten Antwort an, welche zwischen 0 und der erwarteten Anzahl richtiger Antworten liegt.
Abschließend wird die Anzahl gegebener Antworten in der generierten Antwort angegeben.\\

Während der Evaluation stellte sich schnell heraus, dass eine Bewertung der Llama Modelle schwieriger als des GPT4 Modells ist.
Grund hierfür ist das fehlende Training auf ein Stopp-Zeichen.
Die Llama-Modelle enthalten kein Finetuning auf die Beantwortung von Fragen und generieren einer zuvor gegebene Anzahl an Tokens.
Diese Anzahl wurde auf 512 Tokens gesetzt.
Größtenteils wurde die Frage jedoch in einer kleineren Länge beantwortet.
Überbleibende Tokens enthielten Wiederholung bereits generiertem, Formatierungsfehler (wie nicht existente Überschriften), der Beginn neuer Themen oder eine Imitierung neuer Fragestellungen.
Da diese zusätzlichen Tokens eine Ursache des nicht vorhandenen Finetunings ist und nicht zur inhaltlichen Wiedergabe des Modells gehört, wurde die Antwort nur bis zu den genannten Stellen gewertet.
Keine der generierten Antworten benötigte eine größere Länge als 512 Tokens, weshalb einer Verlängerung hier nicht nachgegangen wurde.\\

Sowohl die Antwort-Dateien der Modelle als auch die Bewertungsdateien folgen dem selben Schema der Eingabedatei aus \cref{subsec:eval-dataset}. Durch die Einführung der Eigenschaft \enquote{type} ist eine Unterteilung der Dateien in Fragetypen nicht notwendig, weshalb pro Modell 4 Dateien existieren.
Zwei Dateien, welche die generierten Antworten der Fragen enthalten (für jeden Datensatz, wobei der zweite Datensatz die selben Fragen mit Rechtschreibfehler repräsentiert) und zwei Dateien, welche die Bewertung der generierten Antworten enthalten.
Die Bewertungen erhielten 3 weitere Eigenschaften:

\begin{itemize}
    \item \textbf{answered}: Gibt an, ob die Frage richtig, falsch oder nicht beantwortet wurde
    \item \textbf{points}: Gibt die Anzahl richtiger Antworten in der generierten Antwort an
    \item \textbf{total\_answers}: Gibt die Anzahl gegebener Antworten in der generierten Antwort an
\end{itemize}
