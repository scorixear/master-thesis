...
\todo{full space in german dates (hier in den Fußnoten) sieht komisch aus. ich würde entweder half space (7.\,3.\,2023) oder gar kein Leerzeichen benutzen. Wenn das oft genug vorkommt kannst du auch z.B. das datetime package benutzen aber für ein paar mal lohnt sich das wohl nicht.
Ich empfehle den half space, da urldate in deinem Literaturverzeichnis ja genauso formatiert wird.
}
...
\todo{semantisches Netz und Ontologie werden in diesem Fall ja synonym verwendet, ich würde den Begriff semantisches Netz hier streichen}
\todo{wieso kategorisiert? das ist doch eine einfach besteht-aus Beziehung}
\todo{snik ins abkürzungsverzeichnis}
...
\todo{für später wenn es sich nicht mehr ändert: ich würde einzelne Wörter am Ende eines Blockabsatzes wenn möglich vermeiden, da es visuell unausgewogen wirkt. Gilt allgemein für die ganze Arbeit.}
...
\todo{Ich persönlich finde den Begriff Wissensbasis wie du ihn in der Arbeit gebrauchst verwirrend, da er eine ganz spezielle Bedeutung als Menge von RDF Tripeln im Semantic Web hat, worüber du ja im Rande mit SNIK auch schreibst.
So wie ich das verstehe benutzt du den Begriff eher als allgemeine Ansammlung von Wissen.}
...
\todo{auch hier und generell in dieser Arbeit würde ich den Begriff semantisches Netz komplett streichen, außer beim wörtlichen nennen des ausgeschriebenen titels von SNIK bei der ersten Nennung. Lieber konsistent von Ontologie schreiben.}
Die BeLL mit dem Titel \enquote{Question Answering on SNIK} \citep{hannesbell} erweiterte den Zugang zu diesem Netz durch die Verwendung natürlicher englischer Sprache.\todo{bell ins abkürzungsverzeichnis}
