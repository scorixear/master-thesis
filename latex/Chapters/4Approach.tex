...
Da ein Fine-Tuning in dieser Arbeit aufgrund der fehlenden Datenmenge nicht möglich ist, sollte ein Modell gewählt werden, das auch ohne dieses Fine-Tuning eine gute Performance erreicht.\\
...
\begin{table}
    ...
    \begin{tabular}{lrcccccccc}
        ...
        \bottomrule
    \end{tabular}}
\caption{Zero-Shot Leistung verschiedener Modelle in Begründungsaufgaben für gesunden Menschenverstand. Veröffentlicht in \citet{llama}}\label{tab:llama_naturalquestion}
\end{table}

Die Tabelle~\ref{tab:llama_naturalquestion} zeigt die Leistungsfähigkeit der Modelle im Vergleich zu anderen Modellen und bei verschiedenen Einstellungen (ZeroShot bis 64Shot).
...
Es sollte geprüft werden, ob das Weglassen von Literaturverweisen, Überschriften und Tabellen zu einer besseren Leistung führt.
...
Dabei wird eine Aufteilung von 90\% Training und 10\% Validierung verwendet.
...
PyTorch bietet mit Hilfe von CUDA die Möglichkeit, Trainingsschritte auf einer NVIDIA \ac{gpu} durchzuführen, was die in~\ref{sec:grundlagen:training} beschriebenen Rechenschritte um ein Vielfaches beschleunigt. % TODO: Rechenschritte in Grundlagen erklären, label hinzufügen
...
Diese Parameter sind in der Tabelle~\ref{tab:training:parameter} aufgelistet und folgen den Trainingsparametern von \citet{llama}.
\begin{table}
    \centering
    \begin{tabular}{ll}
        \toprule
...
Diese Anpassung folgt dem Kosinus-Lernratenschema, so dass die endgültige Lernrate 10\% des Anfangswertes beträgt.
...
Zur Ausführung der Trainingsprogramme wird SLURM\footnote{https://slurm.schedmd.com/overview.html (abgerufen am 16.6.2023)} verwendet.
...
Clara ist zweigeteilt und besteht aus 8 Nodes mit je 4 Nvidia Tesla V100 \ac{gpu}s und 22 Nodes mit je 8 Nvidid GeForce RTX 2080 TI \ac{gpu}s.
...
Genauere Erläuterungen zur Konfiguration von DeepSpeed und zur Verwendung der Transformer-Bibliothek sind in~\ref{ch:solution} beschrieben.
...
Dazu werden die in~\ref{sec:approach:questions} definierten Fragen verwendet.
...
Die Modelle sollten immer keine Antwort einer falschen Antwort vorziehen.\\

\noindent\textbf{Determinismus:}\newline
Determinismus beschreibt die Reproduzierbarkeit von Antworten.
Für die gleiche Frage und das gleiche Modell sollten die Antworten immer gleich sein.
