...
Die hier verwendeten Parameter beschreiben ein Training mit \ac{zero} Stufe 2 Optimierung und zusätzlichem \ac{cpu} Offloading.
...
Hier werden zusätzlich Parameter der Modelle auf die \ac{cpu} ausgelagert.
...
Mit Hilfe der Option \texttt{offload\_optimizer} wird der Zustand und die Berechnung des Optimierers auf die \ac{cpu} ausgelagert.
Optional kann dies für sehr große Modelle auch auf eine NVMe SSD erfolgen.
Mit Hilfe der Einstellung \texttt{pin\_memory} wird der für die Berechnung benötigte Speicher auf der \ac{cpu} reserviert.
...
Der Parameter \texttt{train\_micro\_batch\_size\_per\_gpu} beschreibt die Anzahl der Batches pro \ac{gpu}.
...
Sie ermöglicht die Nutzung der \ac{zero}-Optimierung, die es erlaubt, Teile des Trainings und des Modells auf mehrere \ac{gpu}s, der \ac{cpu} und lokalen NVMe SSDs auszulagern.
...
Stattdessen wurde ein \ac{gpu} Computing Cluster des Rechenzentrums der Universität Leipzig verwendet.\\
...
\begin{table}
    ...
    \begin{tabular}{llll}
        ...
        \bottomrule
    \end{tabular}
    \caption[GPU Werte des Rechenzentrums]{ \ac{gpu} Werte des Rechenzentrums der Universität Leipzig}\label{tab:gpu-cluster}
\end{table}
\begin{table}
    \centering
    \begin{tabular}{llll}
...
\end{table}
Das Rechenzentrum der Universität Leipzig stellt zwei \ac{gpu} Computing Cluster mit den Namen \enquote{Paula} und \enquote{Clara} zur Verfügung.
...
Eingeschlossen sind auch alle Kommunikationen, die zwischen den \ac{gpu}s durchgeführt werden, um ihren aktuellen Zustand untereinander abzustimmen.
...
Bei der Ausführung des Trainings auf Nvidia v100 \ac{gpu}s traten in unregelmäßigen Abständen Fehler auf, die in \cref{sec:problem-training} genauer beschrieben sind.
...
Erst die Minimierung der Batchgröße auf 1 pro \ac{gpu}, die Aktivierung des \ac{cpu}-Offloads und die Nutzung von \SI{256}{\giga\byte} \ac{cpu} \ac{ram} ermöglichte ein erfolgreiches Training des Modells Llama 2 7B.
