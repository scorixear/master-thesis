...
def main():
        ...
        for q_type in QuestionType:
            for source in QuestionSource:
                explained_h = heatmap.get_value(
                    name, str(q_type), str(source), "explained"
                )
                if explained_h is None:
                    explained_h = 0
                not_explained_h = heatmap.get_value(
                    name, str(q_type), str(source), "not_explained"
                )
    ...
    create_plot(
        ...
        [data[1] for data in explained],
        "Anzahl der Fragen, die erklärt wurden",
    ...
    for q_type in QuestionType:
        create_plot(
            list(models.keys()),
            [data[0] for data in type_explained[q_type]],
        ...
        show_heatmap(
            ...
            os.path.join(args.output, f"explainability_{q_type}.png"),
        )
    show_heatmap(
    ...
    for source in QuestionSource:
        create_plot(
            list(models.keys()),
            [data[0] for data in source_explained[source]],
        ...
        show_heatmap(
            ...
            "Fragetyp",
            "Modell",
            f"Durchschnittliche Erklärbarkeit der {source} Fragen",
