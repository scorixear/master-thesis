...
def main():
        ...
        for q_type in QuestionType:
            for source in QuestionSource:
                understood_h = heatmap.get_value(
                    name, str(q_type), str(source), "understood"
                )
                if understood_h is None:
                    understood_h = 0
                not_understood_h = heatmap.get_value(
                    name, str(q_type), str(source), "not_understood"
                )
    ...
    create_plot(
        list(models.keys()),
        [data[0] for data in understood],
        [data[1] for data in understood],
        "Verstandene Fragen",
    ...
    for q_type in QuestionType:
        create_plot(
            list(models.keys()),
            [data[0] for data in type_understood[q_type]],
    ...
    for source in QuestionSource:
        create_plot(
            list(models.keys()),
            [data[0] for data in source_understood[source]],
