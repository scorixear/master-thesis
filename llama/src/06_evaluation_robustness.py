...
def main():
    ...
    parser.add_argument("-e", "--eval", type=str, help="Path to the evaluation file",
    ...
    parser.add_argument("-s", "--skip", action="store_true", help="Skip calculation of correctness")
    ...
    show_makrof1_bars(df["MacroF1"].tolist(), model_names, "Makro F1 - Robustheit",
                      args.output + "/makro_total.png")
    for q_type in QuestionType:
        show_makrof1_bars(df[f"MacroF1_{q_type}"].tolist(), model_names,
                          f"Makro F1 ({q_type}) - Robustheit", args.output + f"/makro_{q_type}.png")
    for source in QuestionSource:
        show_makrof1_bars(df[f"MacroF1_{source}"].tolist(), model_names,
                          f"Makro F1 ({source}) - Robustheit", args.output + f"/makro_{source}.png")

    # extract data from correctness evaluation
    makro_eval = evaluated["MacroF1"].tolist()
    show_comparison_bars(df["MacroF1"].tolist(), makro_eval, model_names,
                         "Makro F1 Vergleich - Robustheit", args.output + "/makro_comparison.png")

    for q_type in QuestionType:
        macro_eval = evaluated[f"MacroF1_{q_type}"].tolist()
        show_comparison_bars(df[f"MacroF1_{q_type}"].tolist(), macro_eval, model_names,
    ...
    for source in QuestionSource:
        ...
        show_comparison_bars(df[f"MacroF1_{source}"].tolist(), macro_eval, model_names,
...
def calculate_correctness(args):
    ...
    for index, model in enumerate(models):
        ...
        source_macro_f1: dict[QuestionSource, float] = {
            ...
            QuestionSource.IS_2022_07_18:
                sum(source_f1_scores[QuestionSource.IS_2022_07_18]) / 9, # type: ignore
            QuestionSource.IS_2022_09_27:
                sum(source_f1_scores[QuestionSource.IS_2022_09_27]) / 31, # type: ignore
