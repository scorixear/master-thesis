...
def main():
    ...
    for index, model in enumerate(models):
        ...
        source_macro_f1: dict[QuestionSource, float] = {
            ...
            QuestionSource.IS_2022_07_18: sum(source_f1_scores[QuestionSource.IS_2022_07_18]) / 9, # type: ignore
            QuestionSource.IS_2022_09_27: sum(source_f1_scores[QuestionSource.IS_2022_09_27]) / 31, # type: ignore
    ...
    for type in QuestionType:
        ...
        show_answer_bars(num_correct, num_wrong, num_unanswered, model_names, f"Richtig, Falsch und Unbeantwortete Fragen ({type}) - Robustheit", args.output + f"/answers_{type}.png")
    for source in QuestionSource:
        ...
        show_answer_bars(num_correct, num_wrong, num_unanswered, model_names, f"Richtig, Falsch und Unbeantwortete Fragen ({source}) - Robustheit", args.output + f"/answers_{source}.png")
    ...
    for type in QuestionType:
        show_makrof1_bars(df[f"MacroF1_{type}"].tolist(), model_names, f"Makro F1 ({type}) - Robustheit", args.output + f"/makro_{type}.png")
    for source in QuestionSource:
        show_makrof1_bars(df[f"MacroF1_{source}"].tolist(), model_names, f"Makro F1 ({source}) - Robustheit", args.output + f"/makro_{source}.png")
    ...
    for type in QuestionType:
        macro_eval = evaluated[f"MacroF1_{type}"].tolist()
        show_comparison_bars(df[f"MacroF1_{type}"].tolist(), macro_eval, model_names, f"Makro F1 Vergleich ({type}) - Robustheit", args.output + f"/makro_{type}_comparison.png")
    for source in QuestionSource:
        macro_eval = evaluated[f"MacroF1_{source}"].tolist()
        show_comparison_bars(df[f"MacroF1_{source}"].tolist(), macro_eval, model_names, f"Makro F1 Vergleich ({source}) - Robustheit", args.output + f"/makro_{source}_comparison.png")
