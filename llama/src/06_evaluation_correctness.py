...
def main():
    ...
    parser.add_argument("-s", "--skip", action="store_true",
                        help="Skip recalculation of statistics")
    ...
    show_answer_bars(num_correct, num_wrong, num_unanswered, model_names,
                     "Richtig, Falsch und Unbeantwortete Fragen", args.output+"/answers_total.png")
    show_heatmap(heatmap_data.get_makrof1_heatmap_total_by_model(),
                 "Fragequelle", "Fragetyp", "Makro F1", args.output+"/makrof1_total_model_heat.png")
    for model in model_names:
        show_heatmap(heatmap_data.get_heatmap_by_model(model, "makrof1"),
                     "Fragequelle", "Fragetyp", f"Makro F1 ({model})",
                     args.output+f"/makrof1_{model}_heat.png")

    show_heatmap(heatmap_data.get_makrof1_heatmap_total_by_type(),
                 "Fragequelle", "Modell", "Makro F1", args.output+"/makrof1_total_type_heat.png")
    for q_type in QuestionType:
        ...
        show_heatmap(heatmap_data.get_heatmap_by_type(q_type, "makrof1"),
                     "Fragequelle", "Modell", f"Makro F1 ({q_type})",
                     args.output+f"/makrof1_{q_type}_heat.png")
    show_heatmap(heatmap_data.get_makrof1_heatmap_total_by_source(),
                 "Fragetyp", "Modell", "Makro F1", args.output+"/makrof1_total_source_heat.png")
    for source in QuestionSource:
        ...
        show_heatmap(heatmap_data.get_heatmap_by_source(source, "makrof1"),
                     "Fragetyp", "Modell", f"Makro F1 ({source})",
                     args.output+f"/makrof1_{source}_heat.png")
    # bar plots with macro f1 scores

    show_makrof1_bars(df["MacroF1"].tolist(), model_names, "Makro F1",
                      args.output+"/makro_total.png")
    for q_type in QuestionType:
        show_makrof1_bars(df[f"MacroF1_{q_type}"].tolist(), model_names,
                          f"Makro F1 ({q_type})", args.output+f"/makro_{q_type}.png")
    for source in QuestionSource:
        show_makrof1_bars(df[f"MacroF1_{source}"].tolist(), model_names,
                          f"Makro F1 ({source})", args.output+f"/makro_{source}.png")
...
def calculate_correctness(args):
    ...
    dataframe_columns = ["Model", "Num_Correct", "Num_Wrong",
                         "Num_Unanswered", "Num_Questions", "MacroF1"]
    ...
    for index, model in enumerate(models):
        ...
        for q_type in QuestionType:
            type_macro_f1[q_type] = sum(type_f1_scores[q_type]) / len(type_f1_scores[q_type])
            for source in QuestionSource:
                type_f1: list[int] = heatmap_data.get_value(name, q_type,
                                                            source, "f1") # type: ignore
        ...
        for q_type in QuestionType:
            type_questions[q_type] = type_num_correct[q_type] \
                + type_num_wrong[q_type] + type_num_unanswered[q_type]
        for source in QuestionSource:
            source_questions[source] = source_num_correct[source] \
                + source_num_wrong[source] + source_num_unanswered[source]
