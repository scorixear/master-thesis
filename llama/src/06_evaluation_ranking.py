...
def main():
    ...
    parser.add_argument("-d", "--data", type=str, help="Path to correctness csv file",
    ...
    # num correct
    draw_ranking([x[1] for x in ranked_results["Num_Correct"]],
                 [x[0] for x in ranked_results["Num_Correct"]], "Anzahl korrekter Antworten",
                 "Anzahl korrekter Antworten", f"{args.output}/correct.png")
    draw_ranking([x[1] for x in ranked_results["Num_Unanswered"]],
                 [x[0] for x in ranked_results["Num_Unanswered"]], "Anzahl unbeantworteter Fragen",
                 "Anzahl unbeantworteter Fragen", f"{args.output}/unanswered.png")
    draw_ranking([x[1] for x in ranked_results["MacroF1"]],
                 [x[0] for x in ranked_results["MacroF1"]], "Makro F1", "Makro F1",
                 f"{args.output}/macrof1.png")
    for q_type in QuestionType:
        draw_ranking([x[1] for x in ranked_results[f"Num_Correct_{q_type}"]],
                     [x[0] for x in ranked_results[f"Num_Correct_{q_type}"]],
                     "Anzahl korrekter Antworten", f"Anzahl korrekter Antworten ({q_type})",
                     f"{args.output}/correct_{q_type}.png")
        draw_ranking([x[1] for x in ranked_results[f"Num_Unanswered_{q_type}"]],
                     [x[0] for x in ranked_results[f"Num_Unanswered_{q_type}"]],
                     "Anzahl unbeantworteter Fragen", f"Anzahl unbeantworteter Fragen ({q_type})",
                     f"{args.output}/unanswered_{q_type}.png")
        draw_ranking([x[1] for x in ranked_results[f"MacroF1_{q_type}"]],
                     [x[0] for x in ranked_results[f"MacroF1_{q_type}"]],
                     "Makro F1", f"Makro F1 ({q_type})", f"{args.output}/macrof1_{q_type}.png")
    for source in QuestionSource:
        draw_ranking([x[1] for x in ranked_results[f"Num_Correct_{source}"]],
                     [x[0] for x in ranked_results[f"Num_Correct_{source}"]],
                     "Anzahl korrekter Antworten", f"Anzahl korrekter Antworten ({source})",
                     f"{args.output}/correct_{source}.png")
        draw_ranking([x[1] for x in ranked_results[f"Num_Unanswered_{source}"]],
                     [x[0] for x in ranked_results[f"Num_Unanswered_{source}"]],
                     "Anzahl unbeantworteter Fragen", f"Anzahl unbeantworteter Fragen ({source})",
                     f"{args.output}/unanswered_{source}.png")
        draw_ranking([x[1] for x in ranked_results[f"MacroF1_{source}"]],
                     [x[0] for x in ranked_results[f"MacroF1_{source}"]],
                     "Makro F1", f"Makro F1 ({source})", f"{args.output}/macrof1_{source}.png")
...
def draw_ranking(data: list[int], labels: list[str], y_value: str, title: str, file_name: str):
    ...
    axis.set_xticks(x_ticks, fontsize=PlotParams.font_size)
